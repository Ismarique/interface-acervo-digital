...
function ListagemAlunos(): JSX.Element {
    ...
    const handleRemoverAluno = async (id_aluno: number) => {
        const confirmar = window.confirm("Você realmente deseja remover este registro?(Essa ação é irevercivel)");
        if (confirmar) {
            try {
                            ...
                            {currentAlunos && currentAlunos.length > 0 ? (
                                currentAlunos.map((aluno) => (
                                    <tr className="text-center md:text-left transition-colors hover:bg-slate-50 group" key={aluno.id_aluno}>
                                        <td className="p-3 md:p-4 text-slate-500">{aluno.id_aluno}</td>
                                        ...
                                        <td className="p-3 md:p-4 text-slate-600">{aluno.email}</td>
                                        <td className="p-3 md:p-4 text-slate-600">{aluno.celular}</td>
                                        <td className="p-2 md:p-4">
                                            <div className="flex flex-col sm:flex-row items-center justify-center gap-1 md:gap-2">
                                                ...
                                                onClick={() => navigate(`/detalhes/aluno/${aluno.id_aluno}`)}>Detalhes</button>
                                                ...
                                                <button className="w-full sm:w-auto bg-red-100 text-red-700 px-3 py-1.5 rounded-md text-xs md:text-sm font-medium hover:bg-red-600 hover:text-white transition-all"
                                                 onClick={() => aluno.id_aluno && handleRemoverAluno(aluno.id_aluno)}>Deletar</button>
                                            </div>
                                        </td>
                                    </tr>
                                ))

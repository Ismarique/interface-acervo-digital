    ...
    return (
        <main className="bg-gray-200 flex-1 flex flex-col px-4 sm:px-6 md:px-10 py-6 md:py-10 overflow-hidden">
            <div className="w-full max-w-7xl mx-auto flex flex-col sm:flex-row items-center gap-4 mb-6 md:mb-8 flex-shrink-0">
                ...
                <a href="/cadastro/livro" className="w-full sm:w-auto px-4 py-2 md:px-6 md:py-3 text-sm md:text-base bg-slate-700 rounded-md text-center text-white font-bold flex items-center justify-center hover:cursor-pointer hover:bg-slate-600 transition-all shadow-md hover:shadow-lg active:scale-95">
                    Novo Livro
                </a>
            </div>
            ...
            <div className="w-full max-w-7xl mx-auto flex-1 flex flex-col min-h-0 bg-white rounded-xl shadow-xl border border-slate-300 overflow-hidden">
                <div className="overflow-x-auto overflow-y-auto overscroll-none">
                    <table className="table-auto w-full min-w-[700px] border-collapse text-xs sm:text-sm md:text-base">
                        <thead className="bg-slate-700 sticky top-0 z-10 shadow-sm">
                            <tr>
                                ...
                                <th className="border-b border-slate-600 text-white p-3 md:p-4 text-left">ISBN</th>
                                <th className="border-b border-slate-600 text-white p-3 md:p-4 text-center">Quantidade</th>
                            ...
                            {currentLivros && currentLivros.length > 0 ? (
                                currentLivros.map((livro) => (
                                    <tr className="text-center md:text-left transition-colors hover:bg-slate-50 group" key={livro.id_livro}>
                                        <td className="p-3 md:p-4 text-slate-500">{livro.id_livro}</td>
                                        <td className="p-3 md:p-4 font-medium text-slate-700">{livro.titulo}</td>
                                        <td className="p-3 md:p-4 text-slate-700">{livro.autor}</td>
                                        <td className="p-3 md:p-4 text-slate-600">{livro.isbn}</td>
                                        <td className="p-3 md:p-4 text-center text-slate-600">{livro.quant_disponivel}</td>
                                        <td className="p-2 md:p-4">
                                            <div className="flex flex-col sm:flex-row items-center justify-center gap-1 md:gap-2">
                                                ...
                                                onClick={() => navigate (`/detalhes/livro/${livro.id_livro}`)}>Detalhes</button>
                                                ...
                                                onClick={() => livro.id_livro && handleRemoverLivro(livro.id_livro)}>Deletar</button>
                                            </div>
                                        </td>
                                    </tr>
                                ))
